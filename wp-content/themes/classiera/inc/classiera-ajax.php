...
function make_offer_classiera(){	
	...
	if(empty($offer_author_id)){
		...
		$message = esc_html__( 'You can not send message to yourself', 'classiera' );
	...
	if(isset($_POST['commentID'])){		
		...
		if($getfirstComment){
			...
			foreach ( $getfirstComment as $offerinfo ) :
				...
				//Main Comment//
				$mainHTML = '<div class="classiera_user_message"><a href="#"><img class="img-circle classiera_user_message_img" src="'.$offerAuthorIMG.'" alt="'.$offerAuthor.'"></a><div class="classiera_user_message__box"><p>'.$offer_comment.'</p><p class="classiera_user_message__time">'.$OfferDate.'</p></div></div>';
				//Main Comment//
				//Get Sub Comments//
	...
	if(isset($_POST['main_comment_ID'])){
		...
		if(empty($commentData) || $commentData == ''){
			...
			$replyMessage = array(
				...
				'post_author_id' => $offer_author_id,
				'offer_author_id' => $post_author_id,
		...
		if($commentsData){
			foreach($commentsData as $info){
				...
				echo '<div class="modal-body classiera_show_reply" id="'.$thiscommentID.'">';
					...
					echo '</div>';
					//First Message Box//
					$subComments = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}classiera_inbox_meta WHERE main_comment_id = $thiscommentID AND post_author_id <> 0 ORDER BY id ASC" );
					if($subComments){
						foreach($subComments as $info){
							$reply_check = $info->reply_check;
							$post_author_id = $info->post_author_id;
							...
							$innerDate = date_i18n($dateFormat,  strtotime($innerDate));
							if($reply_check == 'user'){
								$user = get_the_author_meta('display_name', $post_author_id );
								if(empty($user)){
									$user = get_the_author_meta('user_nicename', $post_author_id );
								}
								if(empty($user)){
									$user = get_the_author_meta('user_login', $post_author_id );
								}
								$userIMG = get_user_meta($post_author_id, "classify_author_avatar_url", true);
								$userIMG = classiera_get_profile_img($userIMG);
								if(empty($userIMG)){
...
/*========================== Wang
 Get Logo Image Url
============================ Xing*/
add_action( 'wp_ajax_classiera_get_logo_url', 'classiera_get_logo_url');
add_action( 'wp_ajax_nopriv_classiera_get_logo_url', 'classiera_get_logo_url');
function classiera_get_logo_url(){
	echo get_template_directory_uri() . "/images/logo.svg";
	die();
}
/*=========================== Wang
 classiera_age_over_enter
============================= Xing*/
add_action( 'wp_ajax_classiera_age_over_enter', 'classiera_age_over_enter');
add_action( 'wp_ajax_nopriv_classiera_age_over_enter', 'classiera_age_over_enter');
function classiera_age_over_enter(){
	if ( ! empty( $_SERVER['HTTP_CLIENT_IP'] ) ) {
		//check ip from share internet
		$iuserIp = $_SERVER['HTTP_CLIENT_IP'];
