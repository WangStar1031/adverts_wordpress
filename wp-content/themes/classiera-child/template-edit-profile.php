...
if ($user_ID){
	...
	if ( isset($_FILES['upload_attachment']) ) {
		...
		$path_parts = pathinfo($_FILES['upload_attachment']['name']);
		$type = $path_parts['extension'];
		$target_file = $path . "/" . $user_ID . "_avatar_" . time() . "." . $type;
		// print_r($path);
		// echo "<br>";
		// print_r($target_file);
		// exit();

		if( move_uploaded_file($_FILES['upload_attachment']['tmp_name'], $target_file)){
			$arrUrlPath = explode("/", $_SERVER['REQUEST_URI']);
			$urlPath = "";
			for( $i = 0; $i < count($arrUrlPath) - 2; $i++){
				$urlPath .= $arrUrlPath[$i] . "/";
			}
			$urlPath .= "wp-content/uploads/" . date("Y") . "/" . $month . "/" . $user_ID . "_avatar_" . time() . "." . $type;
			update_user_meta( $user_ID, 'classify_author_avatar_url', $urlPath);
		}
...
<script>
	...
	var src = document.getElementById("file-1");
	var target = document.getElementsByClassName("author-avatar")[0];
