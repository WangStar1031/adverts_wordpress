...
function overlayLogo($_fName){
    ...
    switch($type){
        ...
        case 'jpg': $_imgSrc = imagecreatefromjpeg($_fName); break;
    ...
    // $sx = imagesx($_imgLogo);
    // $sy = imagesy($_imgLogo);
    // $left = imagesx($_imgSrc) - $sx - $margin_right;
    // $top = imagesy($_imgSrc) - $sy - $margin_bottom;
    // imagecopy($_imgSrc, $_imgLogo, $left, $top, 0, 0, $sx, $sy);
    ...
    switch($type){
        ...
        case 'jpg': $retVal = imagejpeg( $_imgSrc, $_fName); break;
        case 'png': $retVal = imagepng( $_imgSrc, $_fName); break;
...
function classiera_filter_posts_columns( $columns ) {
  ...
  // $columns['price'] = __( 'Images Verified', 'classiera' );
  // $columns['area'] = __( 'Age Verified', 'classiera' );
  $columns = array(
      'cb' => $columns['cb'],
      // 'image' => __( 'Image' ),
      'title' => __( 'Advert Title' ),
      'author' => __('Publisher'),
      'time' => __('Display Time'),
      'ID' => __('ID'),
