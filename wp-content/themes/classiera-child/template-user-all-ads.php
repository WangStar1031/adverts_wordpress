										...
										<div class="media">
											...
											<div class="row">
												...
												<div class="col-lg-9 col-xs-12" style="margin-top: 10px">
													...
													$reactivate_post_page_id = $redux_demo['re_activate'];
													if(function_exists('icl_object_id')){
														$templateEditAd = 'template-edit-post.php';
														$templateReActivate = 'template-re-activate.php';	
														$edit_post_page_id = classiera_get_template_url($templateEditAd);
														$reactivate_post_page_id = classiera_get_template_url($templateReActivate);
													}
													$postID = $post->ID;
													global $wp_rewrite;
													if ($wp_rewrite->permalink_structure == ''){
														...
														$reActivate = $reactivate_post_page_id."?reactivate_id=".$post->ID;										
													}else{
													...
													<!-- Re-activate Advert -->
													<?php if(($postStatus == 'draft') && $classiera_ads_status == 1	) { ?>
														<a href="<?php echo esc_url($reActivate); ?>" class="btn btn-primary edit-ad-btn">
															<?php esc_html_e( 'Re-activate', 'classiera' ); ?>
														</a>
													<?php } else {?>
													<!-- / Re-activate Advert -->

													<a href="<?php echo esc_url($edit_post); ?>" class="btn btn-primary edit-ad-btn"><?php esc_html_e("Edit", 'classiera') ?></a>
													<?php } ?>

													<!-- Delete Modal -->
													<a class="btn btn-primary edit-ad-btn" href="javascript:void(0)" data-toggle="modal" data-target="#deleteAdvert-<?php echo $post->ID;?>"></i><?php esc_html_e("Remove", 'classiera') ?></a>
													...
													<?php if($classiera_ads_status == 1 && $postStatus == 'publish') { ?>
														<!-- set time pause-->
														<a href="javascript:void(0)" class="btn btn-primary edit-ad-btn" data-toggle="modal" data-target="#myModal-<?php echo $post->ID;?>"><?php esc_html_e("Pause", 'classiera');?></a>

														<!-- Modal -->
														<div class="modal fade" id="myModal-<?php echo $post->ID;?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
														  <div class="modal-dialog" role="document">
														    <div class="modal-content">
														      <div class="modal-header">
														        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
