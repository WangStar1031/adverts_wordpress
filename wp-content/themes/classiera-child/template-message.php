								...
								<div class="tab-content">
									...
									<div role="tabpanel" class="tab-pane" id="sent">
										<div class="row">
											...
											<div class="col-sm-4" style="background: #2c3e50;">
												<div class="row">
													<ul class="nav nav-tabs nav-stacked sent_comments" style="height: 450px; max-height: 450px; overflow: hidden;">
													<?php 
														$count = 1;
														...
														$avatar = get_avatar_url($post_author_id);
														if($count == 1){
													...
													?>
														...
														<li <?php if($count == 1){ ?> class="active" <?php }?>>
			                                                <a class="user_comment comment_sent" href="#" id="<?php echo esc_attr( $commentID ); ?>" style="padding-left: 15px!important;border-bottom: none !important;border-right: 2px solid;">
			                                                    <img class="user_comment_img thumbnail" src="<?php echo $avatar; ?>" alt="author">
											...
											<div class="col-sm-8" style="background: #e6eaea; height: 450px;">
									...
									<div role="tabpanel" class="tab-pane active" id="recieve">
										<div class="row">
											...
											<div class="col-sm-4" style="background: #2c3e50;">
												<div class="row">
													<ul class="nav nav-tabs nav-stacked received_comments" style="height: 450px; max-height: 450px; overflow: hidden;">
													<?php 
														$count = 1;
													...
													?>
														<!--start loop-->
														<li <?php if($count == 1){ ?> class="active" <?php }?>>
			                                                <a class="user_comment comment_rec" href="#" id="<?php echo esc_attr( $offer_post_id ); ?>" style="padding-left: 15px!important;border-bottom: none !important;border-right: 2px solid;">
			                                                    <img class="user_comment_img thumbnail" src="<?php echo $avatar; ?>" alt="author">
			                                                    <div class="user_comment_body">
												...
												<div class="tab-content classiera_replace_comment">
													...
													?>											
														...
														<div class="user_comment_box <?php echo esc_attr( $commentClass); ?>">
															<a href="#" id="<?php echo esc_attr( $thiscommentID); ?>" class="user_comment user_comment_inner" onclick="aTagClicked(<?=$thiscommentID?>)">
														...
														<div class="user_received_msg_box" idx="<?php echo esc_attr( $thiscommentID); ?>" style="display: none;">
...
<style type="text/css">
	.classiera_comment_ajax_rec, .classiera_comment_ajax{
		background: #e6eaea;
	}
	.user_inbox_content .user_comment:hover {
		background: #32465a;
	}
	ul.nav li a.user_comment{
		background: #2c3e50;
	}
	li.active a.user_comment{
		background: #32465a!important;
	}
	.classiera_user_message__box::before{
	    border-right: 10px solid #435f7a;
	}
	...
	.classiera_user_message__reply .classiera_user_message__box p{
		color: #32465a!important;
	}
	.classiera_user_message.classiera_user_message__reply p.classiera_user_message__time{
...
<script type="text/javascript">
	function callAjaxFunc(){
		var boxes = jQuery(".user_received_msg_box");
		for( var i = 0; i < boxes.length; i++){
			var curBox = boxes.eq(i);
			var idx = curBox.attr("idx");
			var data = {
				'action': 'classiera_get_comment_ajax',
				'commentID': idx
			}
			jQuery.post(ajaxurl, data, function(response){
				var startPos = response.indexOf('id="');
				if( startPos != -1){
					var endPos = response.indexOf('"', startPos + 4);
					var subString = response.substring(startPos + 4, endPos);
					jQuery(".user_received_msg_box[idx=" + subString + "]").html(response);
					jQuery(".user_received_msg_box[idx=" + subString + "]").find(".modal-header").html("");
				}
			});
		}
	}
	callAjaxFunc();
	function aTagClicked(_thiscommentID){
		var arrMsgBox = jQuery(".user_received_msg_box");
		for( var i = 0; i < arrMsgBox.length; i++){
			var curBox = arrMsgBox.eq(i);
			if( curBox.attr("idx") == _thiscommentID){
