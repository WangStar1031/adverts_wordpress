...
class SIRSC_Image_Regenerate_Select_Crop {
	...
	private function init() {
		...
		if ( is_admin() ) {
			add_action( 'init', array( $called, 'maybe_save_settings' ), 1 );
		...
		add_action( 'update_option_sirsc_override_medium_size', array( $called, 'on_update_sirsc_override_size' ) );
						...
						<tbody id="the-list">
							...
							<?php if ( ! empty( $all_sizes ) ) : ?>
								<?php foreach ( $all_sizes as $k => $v ) : ?>
									...
									?>
									<tr id="sirsc-settings-for-<?php echo esc_attr( $k ); ?>" class="hentry <?php echo esc_attr( $clon ); ?>">
	...
	public static function sirsc_override_medium_size_callback() {
		...
		<label><input name="sirsc_override_medium_size" id="sirsc_override_medium_size"
			...
			<?php checked( 1, get_option( 'sirsc_override_medium_size' ) ); ?>/> <?php esc_html_e( 'Crop medium image to exact dimensions (normally medium images are proportional)', 'sirsc' ); ?></label>
		<?php
	}
	...
	public static function sirsc_override_large_size_callback() {
		...
		<label><input name="sirsc_override_large_size" id="sirsc_override_large_size"
			type="checkbox" value="1" class="code"
			<?php checked( 1, get_option( 'sirsc_override_large_size' ) ); ?>/> <?php esc_html_e( 'Crop large image to exact dimensions (normally large images are proportional)', 'sirsc' ); ?></label>
		<?php
	}
	...
	public static function sirsc_use_custom_image_sizes_callback() {
		...
		<table class="widefat fixed striped">
			...
			<tbody>
				<?php if ( ! empty( $all['number'] ) ) : ?>
					<?php for ( $i = 0; $i < (int) $all['number']; $i ++ ) : ?>
						<?php
						$name   = ( ! empty( $all['sizes'][ $i ]['name'] ) ) ? $all['sizes'][ $i ]['name'] : '';
						$width  = ( ! empty( $all['sizes'][ $i ]['width'] ) ) ? (int) $all['sizes'][ $i ]['width'] : 0;
						$height = ( ! empty( $all['sizes'][ $i ]['height'] ) ) ? (int) $all['sizes'][ $i ]['height'] : 0;
						$crop   = ( ! empty( $all['sizes'][ $i ]['crop'] ) ) ? (int) $all['sizes'][ $i ]['crop'] : 0;
						?>
						<tr>
							...
							<td>
								...
								<a href="<?php echo esc_url( admin_url( 'options-general.php?page=image-regenerate-select-crop-settings' ) ); ?>#sirsc-settings-for-<?php echo esc_attr( $name ); ?>"><?php esc_html_e( 'See/manage other settings', 'sirsc' ); ?></a>
							</td>
						</tr>
					<?php endfor; ?>
				<?php endif; ?>
	 ...
	 */
	public static function on_update_sirsc_override_size() {
		...
		if ( ! empty( $all['sizes'] ) ) {
			...
			foreach ( $all['sizes'] as $i => $value ) {
				...
				$all['sizes'][ $i ]['height'] = abs( (int) $value['height'] );
